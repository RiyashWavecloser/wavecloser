...
export default function Users({ users, setUsers, onCreateUser, onUpdateUser, onDeleteUser }) {
  ...
  function stepForward() {
    if (!sel || sel.stage >= 7 || !onUpdateUser) return;
  ...
  return (
    <div style={S.wrap}>
      <PageHeader title="Users (Orca)" subtitle="All Wave Closers members — search, filter, manage" />
                    ...
                    <tr key={u.id} onClick={() => setSelected(u.id === selected ? null : u.id)}
                      ...
                      <td style={S.td}>
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          <div style={S.stagebar}><div style={{ ...S.stagefill, width:`${(u.stage/7)*100}%` }} /></div>
                          <span style={{ fontSize:12, color:'#666' }}>{u.stage}/7</span>
                        </div>
                      </td>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7}>
                    {users.length === 0
                      ...
                      : <EmptyState icon="🔍" title="No users match this filter" message="Try clearing the search or changing the type/stage filter." />}
                  </td></tr>
                )}
              </tbody>
            </table>
              ...
              <div style={{ marginTop:16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={stepForward} disabled={sel.stage >= 7} style={{ padding: '2px 8px', fontSize: 11, background: 'var(--color-primary)', color: 'white', border: '1px solid var(--color-primary)', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>→</button>
                  </div>
                </div>
