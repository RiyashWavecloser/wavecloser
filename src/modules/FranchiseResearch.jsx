...
import { FRANCHISE_MARKETS, PREVIOUS_RESEARCH } from '../data/seed.js';
...
export default function FranchiseResearch() {
  ...
  const [cache,     setCache]     = useState(PREVIOUS_RESEARCH); // last 5 queries
  const [lastQuery, setLastQuery] = useState('Example data');
...
Include exactly 5 markets ordered by score descending. Verdict must be one of exactly: Strong, Moderate, Weak.`,
    ...
    try {
      ...
      setError('Could not parse Claude response — showing example data. Try a simpler query.');
      setResults([...FRANCHISE_MARKETS]);
