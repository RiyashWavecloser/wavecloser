...
export default function OnboardingFlow({ users, setUsers, onCreateUser, onUpdateUser }) {
  ...
  function simulate() {
    ...
    const interval = setInterval(() => {
      ...
      if (stage > 7) {
        ...
        if (onUpdateUser) {
          onUpdateUser(id, { stage: 7 });
        } else {
          setUsers((prev) => prev.map((u) => (u.id === id ? { ...u, stage: 7 } : u)));
        }
  ...
  return (
    <div style={styles.wrap}>
      <PageHeader
        title="Onboarding Flow (Orca)"
        subtitle="7-step CX onboarding — Mildred owns all steps"
      />
      ...
      <Card>
        <CardHeader title="The 7-step CX onboarding flow" sub="Mildred (CX) owns all 7 steps" />
          ...
          {ONBOARDING_STAGES.map((stage, idx) => {
            ...
            return (
              <React.Fragment key={stage.id}>
                ...
                {stage.note && (
                  <div style={{ fontSize: 10, color: '#D49A2B', marginTop: 3, fontStyle: 'italic' }}>⚠ {stage.note}</div>
                )}
                </div>
        ...
        <Card>
          ...
          <ul style={styles.cleanList}>
            {ONBOARDING_STAGES.map(stage => (
              <li key={stage.id} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={styles.numDot}>{stage.id}</span>
                  {stage.label}
                </div>
                {stage.note && (
                  <div style={{ fontSize: 11, color: '#D49A2B', marginLeft: 32, fontStyle: 'italic' }}>⚠ {stage.note}</div>
                )}
              </li>
            ))}
          </ul>
          <Note>Steps 3–7 are pending confirmations from William (see open items #5, 6, 12, 13, 14).</Note>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader
          ...
          sub={`${users.filter(u => u.stage < 7).length} users currently in stages 1–6`}
        />
        {users.filter(u => u.stage < 7).length === 0 ? (
          <Note>All users have passed onboarding — none in stages 1–6.</Note>
        ) : (
          <table style={styles.table}>
            ...
            <tbody>
              {users.filter(u => u.stage < 7).map(u => {
                ...
                return (
                  <tr key={u.id} style={stalled ? { background:'var(--color-amber-bg)' } : {}}>
                    ...
                    <td style={styles.td}>
                      <div style={{ fontSize:13, fontWeight:600 }}>Step {u.stage} / 7</div>
