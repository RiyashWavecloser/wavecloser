...
/**
 * Module 6 — Merchant Lead Generation
 *
 * William has cold-calling agents who each need leads per day.
...
export default function LeadGeneration({ users = [], setUsers, setLeadBadge }) {
  ...
  const [stats, setStats] = useState({ daily: [], topMarkets: [], topBusinessTypes: [] });
  ...
  return (
    <div style={S.wrap}>
      ...
      <PageHeader title="Merchant Lead Generation" subtitle="Generate scored leads for cold-calling agents — POS system prospects" />
      ...
      {tab === 'agents' && (
        <Card>
          ...
          <div style={S.agentGrid}>
            {AGENTS.map(agent => {
              const as = agentStats.find(a => a.agent === agent.name) || {
                agent: agent.name,
                leadsAssigned: 0,
                callsToday: 0,
                interested: 0,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Card>
            ...
            <div style={{ height: 300 }}>
              <ResponsiveContainer>
                <BarChart data={agentStats} margin={{ top: 10, right: 16, left: -8, bottom: 0 }}>
