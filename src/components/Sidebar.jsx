...
const ALL_NAV = [
  ...
  { id: 'users',              label: 'Users (Orca)',                icon: '◉' },
  { id: 'onboarding',         label: 'Onboarding Flow (Orca)',      icon: '⇢' },
  { id: 'automation',         label: 'AI Automation (Orca)',        icon: '✦' },
  { id: 'franchise',          label: 'Merchant Market Research',    icon: '◈' },
  { id: 'leads',              label: 'Merchant Lead Generation',    icon: '⚡' },
  { id: 'recruiter-portal',   label: 'Recruiting Pipeline',         icon: '◎' },
...
export default function Sidebar({ view, setView, dataMode, user, onLogout, leadBadge }) {
  ...
  const role = user?.role || ROLES.PM;
  const roleLabel = ROLE_LABELS[role] || 'Project Manager';
  const allowedViews = ROLE_VIEWS[role] || ROLE_VIEWS[ROLES.PM];
          ...
          {nav.map(item => (
            ...
            >
              ...
              {item.id === 'leads' && leadBadge > 0 && (
                <span style={S.badge}>{leadBadge}</span>
              )}
            </button>
          ))}
        </nav>
...
const S = {
  ...
  badge:     { marginLeft: 'auto', background: 'var(--color-red, #D44A4A)', color: 'white', fontSize: 10, fontWeight: 'bold', padding: '2px 6px', borderRadius: 10, minWidth: 16, textAlign: 'center' },
  ...
  logoutBtn: { background: 'transparent', border: 'none', color: '#888', fontSize: 16, cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'color 0.2s', outline: 'none' },
  hamburger: { position: 'fixed', top: 12, left: 12, width: 40, height: 40, background: '#1A1A1A', color: '#FFF', border: '1px solid #333', borderRadius: 6, fontSize: 20, cursor: 'pointer', zIndex: 40, display: 'none', alignItems: 'center', justifyContent: 'center' },
  backdrop:  { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 44 },
  closeBtn:  { display: 'none', position: 'absolute', top: 12, right: 12, background: 'transparent', border: 'none', color: '#888', fontSize: 24, cursor: 'pointer' },
};
